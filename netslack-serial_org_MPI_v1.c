...
void calculate_no_of_elements_per_process(int n,int p,int * dist);
void calculate_displacement(int elements_on_level,int nprocs,int * displs);
...
void master(char *argv[]) {
	...
	/*:::::::::::::::::::::::: Slack at nodes :::::::::::::::::::::::::::::::::::*/
        ...
        diff_time = end-start;
        printf("------------------------------------------------\n");
	printf("It took %lf for serial version.\n", diff_time);
        printf("------------------------------------------------\n");
...
void calculate_arrival_time_at_nodes(edge* edgeList[],node* nodeList[], int iNodeList[], double at[])
{
	...
	int index_sinkN=0, index_sourceN=0;	
        ...
        MPI_Datatype NODE_MPI = Init_Type_node_MPI ();
        MPI_Type_commit (&NODE_MPI);
        int* element_dist = (int *)malloc(sizeof(int)*nprocs);
	...
	for (i=0;i<total_iNodes;i++)
	{
        ...
        while(rear_indexQ > front_indexQ)
        {
          ...
          int elements_on_level = (rear_indexQ - front_indexQ) -1;

	  // add who will get whom and displacement functions 
          calculate_no_of_elements_per_process(elements_on_level,nprocs,element_dist);
          calculate_displacement(elements_on_level,nprocs,displs);
         ...
         //Broadcast total number of elements on a given level 
         //scatter here 
         MPI_Scatterv(node_send_PACKAGE,element_dist, displs, NODE_MPI, node_recv_PACKAGE,recv_count, NODE_MPI, 0, MPI_COMM_WORLD);
         // Level by Level Computation 
          ...
          for (i=front_indexQ_temp;i<rear_indexQ_temp;i++) {
            ...
            for(j=0;j<nodeQueue[i]->fanin;j++) {
                ...
                at_temp = at[index_sourceN] + nodeList[index_sourceN]->dn+
                             edgeList[((nodeQueue[i])->fanin_indexList[j])]->de;
                at_max = (at_max > at_temp)?at_max:at_temp;
		not_on_level0 =1;
            } 
        
            if(not_on_level0) at[index_sinkN] = at_max;
         }
        // Gather here
...
void calculate_required_arrival_time_at_nodes(edge* edgeList[],node * nodeList[],int oNodeList[], double rat[])
{
	...
	double rat_temp;
	  ...
	  for (i= front_indexQ_temp;i<rear_indexQ_temp;i++) {
		...
		rat_min = (rat_min < rat_temp)?rat_min:rat_temp;
                not_on_level0=1;
	    }
            if(not_on_level0) rat[index_sinkN] = rat_min;
...
// Calculate the offset to the first element of the array of structures sent with MPI
...
void slave() {
        ...
        MPI_Datatype NODE_MPI = Init_Type_node_MPI ();
        MPI_Type_commit (&NODE_MPI);
        int elements_dist [nprocs];
        int recv_count;
	int elements_on_level;
	int displs[nprocs];

        calculate_no_of_elements_per_process(elements_on_level,nprocs,elements_dist);
	calculate_displacement(elements_on_level, nprocs, displs);

        NODE_IAT* node_send_PACKAGE = NULL;
        NODE_IAT* node_recv_PACKAGE = (NODE_IAT*) malloc (sizeof(NODE_IAT)*(elements_dist[rank]));   // to be changed

        MPI_Scatterv(node_send_PACKAGE, elements_dist, displs, NODE_MPI, node_recv_PACKAGE, recv_count, NODE_MPI, 0, MPI_COMM_WORLD);
        printf("Hello from node %d of %d  recv_count = %d \n",rank,nprocs,recv_count);
}
